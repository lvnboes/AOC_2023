...
process_input(Path) ->
    ...
    {lists:droplast(lists:reverse(split(Seeds, <<" ">>, [global]))),[parse_conv_map(ConvMap) || ConvMap <- split(ConversionMaps, <<"\n\n">>, [global])]
}.

parse_conv_map(ConvMap) -> 
    [Conv, Map] = split(ConvMap, <<":\n">>),
    {split(lists:nth(1, split(Conv, <<" ">>)), <<"-to-">>), [[binary_to_integer(Val) || Val <- split(Line, <<" ">>, [global])] || Line <- split(Map, <<"\n">>, [global])]}.
